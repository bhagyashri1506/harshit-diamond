...
function refreshSlider() {
    ...
    function darkMode() {
        ...
        // darkModeSwitch.forEach(el => el.addEventListener('click', ));

        //Set Color Based on Remembered Preference.
        if (localStorage.getItem(pwaName + '-Theme') == "dark-mode") { for (let i = 0; i < toggleDark.length; i++) { toggleDark[i].checked = "checked" }; document.body.classList.add('theme-dark'); document.body.classList.remove('theme-light'); }
        if (localStorage.getItem(pwaName + '-Theme') == "light-mode") { document.body.classList.add('theme-light'); document.body.classList.remove('theme-dark'); } if (document.body.classList.contains("detect-theme")) { setColorScheme(); }
...
function init_template() {
    ...
    document.querySelectorAll('#root')[0].style.display = "block";
    ...
    //     document.querySelectorAll('#root')[0].appendChild(offlineAlert);
    //     document.querySelectorAll('#root')[0].appendChild(onlineAlert);
    // }
